...
const COUNTRY_TERMS = [
  ["United Kingdom", /\b(united kingdom|great britain|britain|british|scotland|wales|london(?!,\s*kentucky)|major|thatcher|hurd)\b/i],
  ...
  ["Austria", /\b(austria|austrian|vienna|vranitzky)\b/i],
  ...
  ["Spain", /\b(spain|spanish|madrid|barcelona|gonzalez)\b/i],
...
const LEADER_TERMS = [
  ...
  ["Felipe Gonzalez", /\bfelipe gonzalez|gonz[aá]lez\b/i],
...
const TOPIC_TERMS = [
  ...
  ["European Community", /\beuropean community|ec\b/i],
...
const FALSE_WESTERN_TITLE_TERMS = /\b(alleged paris meetings|central european states|london conference on the former yugoslavia|london,\s*kentucky|new england)\b/i;
...
async function fetchText(url) {
  const response = await fetch(url, { headers: { "User-Agent": "Bush41-Western-Europe-FRUS-compiler/1.0" } });
...
function keywordProfile(title, body = "") {
  ...
  const bodyCountries = matchesByGroup(searchText, COUNTRY_TERMS);
  ...
  return {
    ...
    topics,
    matchTerms,
    include: !genericEasternEuropeOnly && titleRelevant && !FALSE_WESTERN_TITLE_TERMS.test(titleText)
  };
}
...
function extractAppContent(html) {
  const match = html.match(/<div class="field-docs-content">([\s\S]*?)<\/div>\s*<div class="field-docs-footnote">/);
...
async function harvestGovInfoGranules() {
  ...
  for (const pkg of GOVINFO_PACKAGES) {
    const html = await fetchText(pkg.contextUrl);
    const packageGranules = extractGovInfoGranules(html, pkg.id);
    packageSummaries.push({ packageId: pkg.id, granules: packageGranules.length });
    granules.push(...packageGranules);
  }
...
function dedupe(records) {
  ...
  for (const record of records) {
    ...
    if (!current || current.sourceKind !== "GovInfo granule") seen.set(key, record);
  }
...
async function main() {
  ...
  const records = dedupe([...govInfo.records, ...app.records]);
  ...
  console.log(JSON.stringify({ records: records.length, govInfo: govInfo.records.length, app: app.records.length }, null, 2));
}
